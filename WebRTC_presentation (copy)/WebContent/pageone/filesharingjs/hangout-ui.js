...
var config = {
    ...
    onRoomFound: function (room) {
        ...
        tr.onclick = function () {
            var tr = this;
            var po=window.localStorage.getItem('org.doubango.identity.display_name');
            hangoutUI.joinRoom({
                roomToken: tr.querySelector('.join').id,
                joinUser: tr.id,
               
                userName: prompt('Enter your name', po)
            });
...
function createButtonClickHandler() {
    ...
    hangoutUI.createRoom({
        userName: prompt('Enter your name', po),
        roomName: (po) + ' // shared via ' + (navigator.vendor ? 'Google Chrome (Stable/Canary)' : 'Mozilla Firefox (Aurora/Nightly)')
    });
    hideUnnecessaryStuff();
}
